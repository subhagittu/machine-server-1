...
function getResponse(input) {
  ...
  if (key === 'time') return `The current server time is: ${new Date().toLocaleTimeString()}`;
  if (key === 'date') return `Today's date is: ${new Date().toLocaleDateString()}`;
  if (responses[key]) return responses[key];
  for (const keyword of Object.keys(responses)) {
    if (key.includes(keyword)) return responses[keyword];
  }
  return `🤔 I don't understand "${input}". Type "help" to see what I can do!`;
}
...
const server = net.createServer((socket) => {
  ...
  socket.on('data', (data) => {
    ...
    console.log(`[${clientAddr}] YOU: ${message}`);
    const reply = getResponse(message);
    socket.write(`SERVER: ${reply}\n`);
    console.log(`[${clientAddr}] SERVER: ${reply.split('\n')[0]}`);
  ...
  socket.on('end', () => console.log(`[-] Client disconnected: ${clientAddr}`));
